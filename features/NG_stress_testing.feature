Feature: Stress testing
  ...
  Scenario: Stress testing data setup NG_Updated
    ...
    Given instance "US91282CCW91" of entity "Instruments" is copied with following values
      | Instance ID | Symbol          | ISIN            | Instrument Type | Discount Curve  |
      | Bond1       | NG_RZ_ST_Bond01 | NG_RZ_ST_Bond01 | Fixed Rate Bond | [Curve1.Symbol] |
    ...
    @ng
  Scenario: Stress testing - Account - Change the existing Risk Model
    ...
    When "Position Update" messages are submitted with following values
      | Instance ID | account            | symbol          | price | participant         | notional |
      | POU1        | [Acc01.Account Id] | NG_RZ_ST_Bond01 | 99.0  | [Acc01.Participant] | 200      |

    Then "Position" messages are filtered by "level,participant,account,symbol,notional" should be
      | Instance ID | participant         | account            | level   | symbol          | notional |
      | POS_Res1    | [Acc01.Participant] | [Acc01.Account Id] | ACCOUNT | NG_RZ_ST_Bond01 | 200.0    |
